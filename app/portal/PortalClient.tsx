...
const CFG = {
  ...
  flare: {
    core: ["/portal/flare_core.webp" + Q],
    wide: ["/portal/flare_wide.webp" + Q],
    speed: { core: 0.12, wide: 0.08 }, // ← パララックス係数（近い＝速い）
    opacity: { core: 0.85, wide: 0.65 },
    /** 位置・サイズ（画面基準） */
    coreStyle: {
      width: 560,
      top: "18vh",
...
function CloudHero({ enable3D }: { enable3D: boolean }) {
  ...
  const flareCoreRef = useRef<HTMLDivElement | null>(null);
  const flareWideRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      raf = requestAnimationFrame(() => {
        ...
        const s = CFG.clouds.speed;
        if (raysRef.current) raysRef.current.style.transform = `translate3d(0, ${y * s.rays}px, 0)`;
        if (farRef.current) farRef.current.style.transform = `translate3d(0, ${y * s.far}px, 0)`;
        if (midRef.current) midRef.current.style.transform = `translate3d(0, ${y * s.mid}px, 0)`;
        if (nearRef.current) nearRef.current.style.transform = `translate3d(0, ${y * s.near}px, 0)`;

        // フレアも少しだけ
        if (flareCoreRef.current)
          flareCoreRef.current.style.transform = `translate(-50%, -50%) translate3d(0, ${
            y * CFG.flare.speed.core
          }px, 0)`;
        if (flareWideRef.current)
          flareWideRef.current.style.transform = `translate(-50%, -50%) translate3d(0, ${
            y * CFG.flare.speed.wide
          }px, 0)`;
      });
    };
    onScroll();
    ...
    >
      ...
      <div
        ...
        style={{
          ...CFG.flare.wideStyle,
          position: "absolute",
          opacity: CFG.flare.opacity.wide,
          mixBlendMode: "screen" as any,
        }}
      >
        <ImgFallback
          sources={CFG.flare.wide}
          alt=""
      ...
      <div
        ...
        style={{
          ...CFG.flare.coreStyle,
          position: "absolute",
          opacity: CFG.flare.opacity.core,
          mixBlendMode: "screen" as any,
        }}
      >
        <ImgFallback
          sources={CFG.flare.core}
          alt=""
          className="w-full h-auto object-contain"
          style={{ position: "static" }}
        />
      ...
      {/* キャッチコピー */}
      <div className="absolute inset-x-0 bottom-10 md:bottom-16 z-30">
        <div
      ...
      {/* 下端グラデ（本文へのつなぎ） */}
      <div className="absolute inset-x-0 bottom-0 h-32 z-30 bg-gradient-to-b from-transparent to-black/60" />
    </section>
  );
}
