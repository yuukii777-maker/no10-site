...
/** ===== Z-ORDER（ここが超重要） ===== */
const LOGO_Z  = 2147483647;          // 最前面（ロゴ）
const FRONT_Z = LOGO_Z - 1;          // ロゴの1つ下（前面の雲・フレア）
const BACK_Z  = 1;                   // section 内の背景など
...
function useWrap() {
  ...
  const setX = (dx: number) => {
    const { a, b } = refs.current;
    if (!a || !b) return;
    const tyA = /translate3d\(0, ([^,]+), 0\)/.exec(a.style.transform)?.[1] ?? "0px";
    const tyB = /translate3d\(0, ([^,]+), 0\)/.exec(b.style.transform)?.[1] ?? "0px";
    a.style.transform = `translate3d(${dx}px, ${tyA}, 0)`;
    b.style.transform = `translate3d(${dx}px, ${tyB}, 0)`;
  };
  return { refs, setH, setPos, setX };
}
...
export default function PortalClient() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      [rays, far, mid, mid2, near, flareWide, flareCore].forEach((w) => w.setX(tmx));
  ...
  return (
    <main className="relative min-h-[300vh] text-neutral-200">
      ...
      {heroOnScreen && (!use2D ? (
        <div
          style={{ position: "fixed", left: 0, top: heroTop, width: "100%", height: heroHeight, zIndex: LOGO_Z, pointerEvents: "none" }}
          aria-hidden
        >
          <ThreeHeroLazy
