    ...
    <div className="wrap" id="rules">
      ...
      <style jsx global>{`
        :root{
          ...
          --accent:#c9a86a; --panel:rgba(10,14,21,.80); --stroke:rgba(201,168,106,.45);
        ...
        .hero-scrim{
          ...
          background: linear-gradient(to bottom, rgba(10,14,21,0.95) 0%, rgba(10,14,21,0.78) 55%, rgba(10,14,21,0.00) 100%);
        }
        @media (max-width:960px){ .hero-scrim{ height:46vh } }

        .hero{text-align:center; margin:18px 0 24px; position:relative; z-index:12;}
        .title{
          margin:0 0 10px; font-size:clamp(34px,3.6vw,56px); letter-spacing:.10em; font-weight:700; line-height:1.1;
          color:#e9edf3; -webkit-text-stroke:.35px rgba(0,0,0,.35); text-shadow:0 1px 2px rgba(0,0,0,.35);
          font-feature-settings:"palt" 1, "kern" 1;
        }
        .tagline{color:var(--muted); margin:0; font-size:clamp(13px,1.2vw,15px)}

        .card{ background: rgba(10,14,21,.5); backdrop-filter: blur(3px) saturate(1.05);
               -webkit-backdrop-filter: blur(3px) saturate(1.05);
               border:1px solid var(--stroke); border-radius:20px; padding:28px; box-shadow:var(--shadow); }
